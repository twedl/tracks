...
TRAFFIC_STOP_DWELL_RANGE = (15.0, 45.0)  # seconds


def classify_and_cap(speed_mps: float) -> float:
    """Cap OSRM speed to truck limits based on road class."""
    if speed_mps > 22.0:  # highway
        return min(speed_mps, HIGHWAY_CAP)
    elif speed_mps > 11.0:  # arterial
...
def smooth_speeds(speeds_mps: list[float], distances_m: list[float]) -> list[float]:
    ...
    Ensures no speed transition exceeds physical limits for a loaded truck.
    Forward pass enforces max acceleration, backward pass enforces max deceleration.
    """
    n = len(speeds_mps)
    if n == 0:
        return speeds_mps
    ...
    for i in range(1, n):
        ...
        if dist < 0.1:
            ...
            continue
        # v^2 = u^2 + 2*a*d => max_v = sqrt(u^2 + 2*a*d)
        max_v = np.sqrt(smoothed[i - 1] ** 2 + 2 * MAX_ACCEL * dist)
    ...
    for i in range(n - 2, -1, -1):
        ...
        if dist < 0.1:
            smoothed[i] = smoothed[i + 1]
            continue
        max_v = np.sqrt(smoothed[i + 1] ** 2 + 2 * MAX_DECEL * dist)
        smoothed[i] = min(smoothed[i], max_v)
...
def inject_traffic_stops(
    ...
    Returns modified speeds and distances lists (with inserted stop segments).
    """
    new_speeds = []
    new_distances = []

    for i, (spd, dist) in enumerate(zip(speeds_mps, distances_m)):
        is_urban = spd < 15.0
        if is_urban and dist > 20 and rng.random() < TRAFFIC_STOP_PROB:
            # Split: approach, stop, resume
            ...
            # Stopped (dwell modeled as zero-speed, tiny distance)
            dwell = rng.uniform(*TRAFFIC_STOP_DWELL_RANGE)
            new_speeds.append(0.0)
            new_distances.append(0.1)  # nominal distance for the stop point
